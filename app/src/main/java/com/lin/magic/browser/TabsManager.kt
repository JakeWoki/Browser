...
import com.lin.magic.Entitlement
import com.lin.magic.R
import acr.browser.lightning.browser.sessions.Session
import com.lin.magic.constant.INTENT_ORIGIN
...
import com.lin.magic.utils.*
import com.lin.magic.view.*
...
import com.lin.magic.Component
...
import com.lin.magic.utils.isIncognitoPageUrl
...
import com.lin.magic.view.TabInitializer
...
class TabsManager @Inject constructor(
    ...
    */
    ...
    private fun finishInitialization() {
        ...
        try {
            if (allTabs.size == savedRecentTabsIndices.size) { // Defensive
                ...
                // See: https://console.firebase.google.com/u/0/project/magic-b1f69/crashlytics/app/android:net.slions.magic.full.playstore/issues/d70a65025a98104878bf2da4aa06287e?time=last-seven-days&sessionEventKey=650AE750014800016260FF77850BA317_1859332239793370743
    ...
    {
        val bundle = com.lin.magic.utils.FileUtils.readBundleFromStorage(application, aFilename)
    ...
    fun renameSession(aOldName: String, aNewName: String) {
        ...
        com.lin.magic.utils.FileUtils.renameBundleInStorage(application, fileNameFromSessionName(oldName), fileNameFromSessionName(aNewName))
    ...
    fun saveState() {
        ...
        // See: https://github.com/Slion/Magic/issues/193
        if (!isInitialized) {
    ...
    private fun saveCurrentSession(aName: String) {
        ...
        iScopeThreadPool.launch {
            ...
            com.lin.magic.utils.FileUtils.writeBundleToStorage(application, outState, temp)
            // Defensively delete session backup, that should never be needed really
            com.lin.magic.utils.FileUtils.deleteBundleInStorage(application, backup)
            // Rename our session file as backup
            com.lin.magic.utils.FileUtils.renameBundleInStorage(application, session, backup)
            // Rename our temporary session to actual session
            com.lin.magic.utils.FileUtils.renameBundleInStorage(application, temp, session)
            ...
            // 2022-01-11 11:32:59.939 23094-23207/net.slions.magic.full.download.debug D/TabsManager: Tick: 28
            // 2022-01-11 11:33:00.224 23094-23208/net.slions.magic.full.download.debug D/TabsManager: Tick: 20
//            repeat(30) {
//                delay(1000L)
...
//            }
     ...
     * browser next starts.
     */
    fun clearSavedState() = com.lin.magic.utils.FileUtils.deleteBundleInStorage(application, FILENAME_SESSION_DEFAULT)
    ...
    fun deleteSession(aSessionName: String) {
        ...
        // Delete session file
        com.lin.magic.utils.FileUtils.deleteBundleInStorage(application, fileNameFromSessionName(iSessions[index].name))
    ...
    fun saveSessions() {
        ...
        iScopeThreadPool.launch {
            // Guessing delay is not needed since we do not use the main thread scope anymore
            //delay(1L)
            com.lin.magic.utils.FileUtils.writeBundleToStorage(application, bundle, FILENAME_SESSIONS)
        }
    }
     ...
     */
    fun deleteSessions() {
        com.lin.magic.utils.FileUtils.deleteBundleInStorage(application, FILENAME_SESSIONS)
    }
    ...
    private fun loadSessions() {
        val bundle = com.lin.magic.utils.FileUtils.readBundleFromStorage(application, FILENAME_SESSIONS)
