    ...
    private fun Activity.intentForScheme(uri: Uri): Intent? {
        ...
        return when (scheme) {
            "mailto" -> {
                ...
                com.lin.magic.utils.Utils.newEmailIntent(mailTo.to, mailTo.subject, mailTo.body, mailTo.cc)
            }
                ...
                if (URLUtil.isFileUrl(url) && !url.isSpecialUrl()) {
                    ...
                    return if (file.exists()) {
                        val newMimeType = MimeTypeMap.getSingleton()
                            .getMimeTypeFromExtension(com.lin.magic.utils.Utils.guessFileExtension(file.toString()))
