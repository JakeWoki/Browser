 ...
 */
package com.lin.magic.download
...
import com.lin.magic.extensions.KDuration
...
import com.lin.magic.utils.guessFileName
...
class LightningDownloadListener     //Injector.getInjector(context).inject(this);
    ...
    val downloadHandler: com.lin.magic.download.DownloadHandler = hiltEntryPoint.downloadHandler
        ...
        if (intent.action == DownloadManager.ACTION_DOWNLOAD_COMPLETE) {
            ...
            if (downloadHandler.iDownloadId == id) {
                ...
                if (!success) {
                    ...
                    downloadsIntent =
                        com.lin.magic.utils.Utils.getIntentForDownloads(mActivity, userPreferences.downloadDirectory)
    ...
    override fun onDownloadStart(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            ...
            if (permissions.all { ContextCompat.checkSelfPermission(mActivity, it) == PackageManager.PERMISSION_GRANTED }) {
                doDownloadStart(url, userAgent, contentDisposition, mimetype, contentLength)
            } else {
                // 检查是否曾拒绝权限
                if (permissions.any { ActivityCompat.shouldShowRequestPermissionRationale(mActivity, it) }) {
                    //TODO show message
                } else {
                    val requestPermissionsLauncher = (mActivity as ComponentActivity).registerForActivityResult(
